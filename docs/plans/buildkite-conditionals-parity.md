...
| `spec/models/build/condition_spec.rb` | `env()`, `build.env()`, build/pipeline/org fields, webhook fields, pull request label, project env merge, validation, context construction | `blocked`: Slice 2 seeds source-tagged root cases for representative `env()`, `build.env()`, organization, pipeline, webhook, pull request label, project env merge, and validation behavior; Slice 5 adds server-style static env validation for typos, unsupported built-ins, invalid names, and names starting with `$`; the exhaustive context matrix is still Slice 5 | `ported` |
